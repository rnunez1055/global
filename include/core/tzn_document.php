...
class TznFile extends Tzn {
  ...
  function TznFile($type='image') {
        ...
        $this->_mimeList["octet-stream"] = ".mime";
	...
	function upload($field) {
		...
		if ((!empty($type)) && (array_key_exists(trim(strtolower($type)), $this->_mimeList))) {
	...
	function thumbnail($width=9999,$height=9999,$folder='thumb') {
	...
	function simpleResize($width=800,$folder='../') {
		...
		$filetemp=$folder.TZN_FILE_UPLOAD_URL.$this->fileName;
		
		$orig_size = @getimagesize($filetemp);
		$wx = $orig_size[0];
		
		if ($wx > $width){			
			$image = new SimpleImage();
   			$image->load($folder.TZN_FILE_UPLOAD_URL.$this->fileName);
   			$image->resizeToWidth($width);
   			$image->save($folder.TZN_FILE_UPLOAD_URL.$this->fileName);
		}		
	}
...
class TznThumbnail	{
	...
	function getInfo() {
		...
		{			
			...
			/*if($this->_orgXSize==1350 || $this->_orgYSize==563){
				$this->_genXSize = $this->_orgXSize;
				$this->_genYSize = $this->_orgYSize;
			}else{			
			...
			$this->_genXSize = $this->_orgXSize;
			$this->_genYSize = $this->_orgYSize;
			//$this->_genXSize = round($this->_orgXSize);
			//$this->_genYSize = round($this->_orgYSize);
