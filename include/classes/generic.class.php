...
class Generic extends TznDb {
	...
	function qEditor($name, $config = array()) {
		...
		switch (strtolower($config['toolbar'])) {
			...
			case "multimedia" : $config['toolbar'] = array(array('Undo','Redo','-','Cut','Copy','Paste','PasteText','PasteFromWord','-','Bold', 'Italic', 'Underline', 'Strike','-','JustifyLeft','JustifyCenter','JustifyRight','JustifyBlock','BulletedList','-','RemoveFormat','-','Image','Link', 'Unlink','-','HorizontalRule' ,'-', 'Source','ShowBlocks','TextColor','BGColor','FontSize','Format','Styles', '-','Table'));
	...
	function pFile($name = 'ifile', $href = "", $field = 'target', $type='img', $req = true) {
		if ($this ->$field) {
			...
			switch ($type) {
				...
				default    : $p .= "<a href='".TZN_FILE_UPLOAD_PATH.$this ->$field."' rel='shadowbox'> " . $this ->$field . "</a><br/>"; break;
			}
			$p .= "<a href='$href' style='color:rgb(211, 16, 31);' title='delete file' >(X) eliminar</a></div>";
			print $p;
		} else {
    ...
    function getUrlEmbedYT($id=null) {
		$id = !$id ? $this ->video : $id  ;
		return "https://www.youtube.com/embed/$id?autoplay=1";
	}
    
    function getThumbYT ($id=null,$w,$h) {
		$id = !$id ? $this ->video : $id  ;
		//return '<img src="'. "http://img.youtube.com/vi/$id/0.jpg" .'" width="'.$w.'" height="'.$h.'"  class="borde"/>';
		return '<img src="'. "http://img.youtube.com/vi/$id/0.jpg" .'" width="'.$w.'" height="'.$h.'"/>';
	}
    
    function pVideo($name = 'video', $href = "") {
		if ($this ->$name) {
			$p  = '<div class="img-border" style="width:130px;" >';
			$p .=  $this ->getThumbYT();
			$p .= "<a href='". $this ->getUrlEmbedYT($this ->{$name})  ."' rel='shadowbox'> " . $this ->{$name} . "</a><br/>"; 
			
			$p .= "<a href='$href' style='color:rgb(211, 16, 31);' title='delete file' >(X) eliminar</a></div>";
			print $p;
		} else {
			$req = $req === false ? '' : "required"; 
			$this ->qText($name, '', '', 'class="url required" style="width:98%;" ');
			print "<br/><small>Video Youtube: ejemplo: https://www.youtube.com/watch?v=8D8YRyW5PNE </small>";
		}
	}
